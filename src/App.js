    ...
    render() {
        return (
            <div className="App">
                <header>
                    <div id="header-content">
                        ...
                        <div className="auth-btns">
                            <button className="btn-1 btn-spotauth"
                                    onClick={() => window.location = 'https://playlist-converter-backend.herokuapp.com/spotify/authenticate/'}>
                                <span>Auth Spotify</span></button>
                            <button className="btn-1 btn-ytauth"
                                    onClick={() => window.location = 'https://playlist-converter-backend.herokuapp.com/youtube/authenticate/'}>
                                <span>Auth YouTube</span></button>
                        </div>
                    </div>
                ...
                <TrackList playlistId={this.state.playlistId} onPlaylistIdChange={this.handlePlaylistIdChange}
                           onProgressChange={this.handleProgressChange}/>
                <br/>
                <div className="destPL"><p className="destPL-text">{this.state.playlistId}</p>
                    <button onClick={() => this.togglePopup()} className="btn-1"><FontAwesomeIcon icon={faEdit}
                                                                                                  size="lg"/><span
                        className="btn-text">New Playlist</span></button>
                </div>
                <br/>
                <div>
                    <button
                        onClick={() => window.open('https://www.youtube.com/playlist?list=' + this.state.playlistId)}
                        className="btn-1 btn-yt"><FontAwesomeIcon icon={faYoutube} size="lg"/><span
                        className="btn-text">Open YouTube Playlist</span></button>
                </div>
                {this.state.showPopup ?
                    <Popup closePopup={this.togglePopup} onPlaylistIdChange={this.handlePlaylistIdChange}/> : null}
            </div>
        );
    }
}
...
class TrackList extends Component {
    ...
    render() {
        let shuffle = <button onClick={() => this.shuffle()} className="btn-1 tool"><FontAwesomeIcon icon={faRandom}
                                                                                                     size="lg"/><span
            className="btn-text">Shuffle</span></button>;
        let clearList = <button onClick={() => this.clearList()} className="btn-1 tool"><FontAwesomeIcon icon={faTrash}
                                                                                                         size="lg"/><span
            className="btn-text">Clear List</span></button>;
        let transfer = <button onClick={() => this.insertPlaylistItems()} className="btn-1 tool"><FontAwesomeIcon
            icon={faExchangeAlt} size="lg"/><span className="btn-text">Transfer Songs</span></button>;
        let settings = <button onClick={() => this.toggleSettings()} className="btn-1 tool"><FontAwesomeIcon
            icon={faSlidersH} size="lg"/><span className="btn-text">Settings</span></button>;
        return (
            <div className="list-main">
                <div className="searchbar">
                    <input type="text" placeholder="Spotify Playlist URI" value={this.state.inputURI}
                           onChange={e => this.setState({inputURI: e.target.value})}/>
                    <button className='submit' onClick={() => this.handleClick()}>Add playlist</button>
                </div>
                <div className="searchbar">
                    <input placeholder="Song - Artist" value={this.state.userInput} type="text"
                    ...
                    <FlipMove className="flipmove" duration={250} easing="ease-out">
                        ...
                        {this.state.list.map(item =>
                            <li onClick={() => this.removeItem(item.key)} key={item.key}>
                                <div className="album col-sm-2 col-xs-3"><img alt={item.name + ' - ' + item.artist}
                                                                              src={item.img}/></div>
                                <div className="metatext col-sm-8 col-xs-6"><p className="trackname">{item.name}</p><p
                                    className="artist">{item.artist}</p></div>
                                <div className="col-sm-2 col-xs-3"><FontAwesomeIcon icon={item.icon} size="lg"
                ...
                {this.state.showSettings ?
                    <Settings closeSettings={this.toggleSettings} list={this.state.list}/> : null}
            </div>
        )
    }
}
            ...
            <div className="popup">
                <div className="popup_inner">
                    <div className="popup_header">
                        ...
                        <button onClick={this.props.closePopup} className="close"><FontAwesomeIcon icon={faTimes}
                                                                                                   size="lg"/></button>
                    </div>
                    {(this.state.body === 'default') ? <div className="popup_body popup_default">
                            <button onClick={() => this.setState({
                                body: 'existingPlaylist',
                                header: 'Use Existing YouTube Playlist'
                            })} className="btn-1 btn-popup"><span>Existing YouTube Playlist</span></button>
                            <p className="hr-text"><span>OR</span></p>
                            <button onClick={() => this.setState({
                                body: 'createPlaylist',
                                header: 'Create New YouTube Playlist'
                            })} className="btn-1 btn-popup"><span>Create New YouTube Playlist</span></button>
                        </div>
                        : null}
                    {(this.state.body === 'existingPlaylist') ?
                        <PopupExistingPlaylist onPlaylistIdChange={this.props.onPlaylistIdChange.bind(this)}
                                               closePopup={this.props.closePopup.bind(this)}/> : null}
                    {(this.state.body === 'createPlaylist') ?
                        <PopupCreatePlaylist onPlaylistIdChange={this.props.onPlaylistIdChange.bind(this)}
                                             closePopup={this.props.closePopup.bind(this)}/> : null}
                </div>
            </div>
        );
        ...
        return (
            <div className="popup_body">
                {this.state.error.show ?
                    <div className="error">
                        <div className="error-header">
                            ...
                            <button onClick={() => this.setState({error: {show: false}})} id="error-close"
                                    className="close"><FontAwesomeIcon icon={faTimes}/></button>
                        </div>
                ...
                <form onSubmit={this.handleSubmit}>
                    <label>
                        <span>Title:</span><br/>
                        <textarea name="title" required maxLength="150" value={this.state.title}
                                  onChange={e => this.setState({title: e.target.value})}/>
                    </label>
                    <br/>
                    <label>
                        <span>Description:</span><br/>
                        <textarea name="description" maxLength="5000" value={this.state.description}
        ...
        return (
            <div className='popup'>
                <div className='popup_inner'>
                    <div className="popup_header">
                        <h3>Preferences</h3>
                        <button onClick={this.props.closeSettings} className="close"><FontAwesomeIcon icon={faTimes}
                                                                                                      size="lg"/>
                        </button>
                    </div>
                    <div className="popup_body">
